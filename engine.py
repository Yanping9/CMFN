...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
                    data_loader: Iterable, optimizer: torch.optim.Optimizer, clip_img_encoder,
                    device: torch.device, epoch: int, max_norm: float = 0):
    model.train()
    criterion.train()
    metric_logger = utils.MetricLogger(delimiter="  ")
    metric_logger.add_meter('lr', utils.SmoothedValue(window_size=1, fmt='{value:.6f}'))
    if hasattr(criterion, 'loss_labels'):#检查对象criterion是否具有属性x
    ...
    elif hasattr(criterion, 'loss_verb_labels'):
    ...
    for samples, targets in metric_logger.log_every(data_loader, print_freq, header):
        ...
        if clip_img_encoder is not None:
            clip_input = torch.stack([v['clip_inputs'] for v in targets])
...
@torch.no_grad()
def evaluate_hoi(dataset_file, model, clip_img_encoder, postprocessors, data_loader,
    ...
    for samples, targets in metric_logger.log_every(data_loader, 200, header):
        ...
        _, clip_input = clip_img_encoder(clip_input)
        # print(targets)
        outputs = model(samples, clip_input=clip_input, is_training=False)
        del clip_input
