...
class GEN(nn.Module):
    ...
    def __init__(self, d_model=512, nhead=8, num_encoder_layers=6,
                 ...
                 return_intermediate_dec=False, verb_aux=False):
        super().__init__()
        ...
        if self.verb_aux:
            ...
            self.verb_decoder = TransformerDecoder(verb_decoder_layer,
                                                   num_dec_layers,
                                                   verb_decoder_norm,
                                                   return_intermediate=return_intermediate_dec)
    ...
    def forward(self, src, mask, query_embed_h, query_embed_o, pos_guided_embed, pos_embed):
        ...
        mem_pos_embed = pos_embed.flatten(2).permute(2, 0, 1)
        ...
        query_embed_o = query_embed_o.unsqueeze(1).repeat(1, bs, 1)#(n_q,dim)->(n_q,1,dim)->(n_q,bs,dim)
        ...
        memory = self.encoder(src, src_key_padding_mask=mask, pos=mem_pos_embed)

        ins_hs = self.instance_decoder(ins_tgt, memory, memory_key_padding_mask=mask,
                                       pos=mem_pos_embed, query_pos=ins_query_embed)
        ...
        if self.verb_aux:
            v_h_hs = h_hs.detach()
            v_o_hs = o_hs.detach()
            v_memory = memory.detach()
            v_mem_pos = mem_pos_embed.detach()
            verb_guided_embed = (v_h_hs + v_o_hs) / 2.0
            verb_guided_embed = verb_guided_embed.permute(0, 2, 1, 3)
            verb_tgt = torch.zeros_like(verb_guided_embed[0])
            verb_hs = self.verb_decoder(verb_tgt, v_memory, memory_key_padding_mask=mask,
                                        pos=v_mem_pos, query_pos=verb_guided_embed)
            verb_hs = verb_hs.transpose(1, 2)
        else:
            verb_hs = None


        return h_hs, o_hs, verb_hs, memory, mem_pos_embed
