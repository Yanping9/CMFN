...
from .create_spatial_union import build_SpatialCreator
from .create_hoifeature import build_hoi_feature
...
class CMFN(nn.Module):
    ...
    def __init__(self, backbone, transformer, spa_creator, hoi_extractor, num_queries, aux_loss=False, args=None):
        super().__init__()
        self.args = args
        ...
        self.obj_logit_scale = nn.Parameter(torch.ones([]) * np.log(1 / 0.07))  # 创建一个可学习标量，初始值
    ...
    def forward(self, samples: NestedTensor, clip_input=None, is_training=True):
        ...
        inter_hs, ins_context = self.hoi_extractor(h_hs.transpose(1, 2), o_hs.transpose(1, 2), mask, memory, mem_pos,
                                                   paired_embed, clip_f, h_pos, o_pos, ho_pos)
        # inter_hs, ins_context = self.hoi_extractor(h_hs[-1].transpose(0, 1), o_hs[-1].transpose(0, 1), mask, memory, mem_pos,
        #                                            paired_embed, clip_f, h_pos, o_pos, ho_pos)
    ...
    @torch.jit.unused
    def _set_aux_loss_triplet(self, outputs_hoi_class, outputs_obj_class, outputs_sub_coord,
                              outputs_obj_coord, outputs_inter_hs=None, outputs_verb_class=None):
        ...
        if outputs_verb_class is not None:
            aux_outputs['pred_verb_logits'] = outputs_verb_class[-self.dec_layers: -1]
        outputs_auxes = []
...
class SetCriterionAUX(nn.Module):
    ...
    def loss_verb_labels(self, outputs, targets, indices, num_interactions, topk=5):
        assert 'pred_verb_logits' in outputs
        src_logits = outputs['pred_verb_logits']
        ...
        for tid, target in enumerate(target_classes_o):
            ...
            acc += acc_pred / len(tgt_idx)
        rel_labels_error = 100 - 100 * acc / max(len(target_classes_o), 1)
        losses['verb_class_error'] = torch.from_numpy(np.array(
            rel_labels_error)).to(src_logits.device).float()
        return losses
    ...
    def forward(self, outputs, targets):
        ...
        num_interactions = sum(len(t['verb_labels']) for t in targets)
        num_interactions = torch.as_tensor([num_interactions], dtype=torch.float,
                                           device=next(iter(outputs.values())).device)
        if is_dist_avail_and_initialized():
            torch.distributed.all_reduce(num_interactions)
...
def build(args):
    ...
    gen = build_gen(args)
    spatialcreator = build_SpatialCreator(args=args)
    ...
    matcher = build_matcher(args)  # 匈牙利匹配器，计算预测框与真实框之间的匹配关系，并返回匹配的结果
    weight_dict = {}
    if args.verb_aux:
    ...
    if args.verb_aux:
        losses = ['verb_labels']
        criterion = SetCriterionAUX(args.num_obj_classes, args.num_queries, args.num_verb_classes, matcher=matcher,
