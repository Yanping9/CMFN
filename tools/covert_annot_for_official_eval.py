...
if __name__ == '__main__':
    #Generate_HICO_detection('./hico_gen.json', './ppdm_results/')
    parser = argparse.ArgumentParser()

    parser.add_argument(
        '--root_path', type=str, default='/home/txs/work/yw',
    )
    parser.add_argument(
        '--json_path', type=str, default='/eval_data/hico/fela_r101_verb/fela_r101_verb_9.0.json',
    )
    parser.add_argument(
        '--save_path', type=str, default='/eval_data/hico/matlab_eval/EMI/EMI_r101_9.0/',
    )

    args = parser.parse_args()
    save_path = args.root_path + args.save_path
    json_path = args.root_path + args.json_path
    if args.save_path:
        Path(args.root_path + args.save_path).mkdir(parents=True, exist_ok=True)
    Generate_HICO_detection(json_path, save_path)